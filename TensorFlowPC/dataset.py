...
def channel_last_transform(x):
    transformed_array = []
    for i, sEMG_array in enumerate(x):
        sEMG_array = sEMG_array.transpose()
        transformed_array.append(sEMG_array)
    
    transformed_array = np.array(transformed_array)
    transformed_array = np.expand_dims(transformed_array, axis=-1)
...
def folder_extract(root_dir, exercises=["E2"], myo_pref="elbow"):
    ...
    # transform semg from -128 - 127 to 0 - 255
...
def realtime_preprocessing(emg, params_path=None, num_classes=4, window=24):
    """
    Purpose:
    ...
    inputs, outputs = apply_window(gesture, window)
